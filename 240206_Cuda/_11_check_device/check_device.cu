...
// 定义一个宏，用于检查 CUDA 操作的返回状态。
// 如果状态不为零（表示出现错误），则打印错误信息并终止程序
#define CHECK(status)                                          \
    do                                                         \
    {                                                          \
        auto ret = (status);                                   \
...
- `Device name: NVIDIA GeForce RTX 4060 Laptop GPU`：设备型号是NVIDIA GeForce RTX 4060 Laptop GPU，专为笔记本电脑设计的显卡。
...
- `MultiProcessor Count: 24`：SM数量，指的是流多处理器（Streaming Multiprocessors）的数量，这是NVIDIA GPU架构中的一个核心组件，用于执行计算任务。
...
- `Total memory (bytes): 4290248704`：总的全局内存大小，以字节计，这里是4290248704字节，大约为4GB。
